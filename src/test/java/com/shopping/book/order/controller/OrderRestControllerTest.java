...
class OrderRestControllerTest {
    ...
    @Test
    @DisplayName("결제완료 후 테스트 - 실패")
    public void testOrderComplete_Fail() throws Exception {
        //given
        OrderCompleteDto orderCompleteDto = new OrderCompleteDto("imp_123456", new Orders());

        doThrow(new RuntimeException("Complete order failed")).when(orderService).completeOrder(any(OrderCompleteDto.class)); //any(OrderCompleteDto.class)
        doNothing().when(orderService).cancelPaymentAndDeleteOrder(orderCompleteDto);
        ...
        mockMvc.perform(post("/api/order/complete")
                ...
                .andExpect(jsonPath("$.error").value("Complete order failed"));
    }
